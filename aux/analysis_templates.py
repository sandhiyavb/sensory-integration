...
def firing_fields_ovc_agent(metadata, weights, images, angle, agent_pos=(0.0,0.0),
                            obj_id='Cylinder', actions=8, radial=False) :
    ...
    if radial : 
        rad   = np.linspace(0, x_limits[1], num=resolution[0])
        theta = np.linspace(0, 2*np.pi, num=resolution[1])
...
#     fields = []
#     for a in angles :
#         firing_fields, x, y = firing_fields_allo(scene, True, network, 
#                                           weights, a)
#         fields.append(firing_fields)
#     data = {'FIELDS' : fields, 'GOAL' : np.array([x,y])}
#     np.save('fields.npy',data)
# 
#     data = np.load('fields.npy').item()
# 
#     draw_cell_mosaics(np.array(data['FIELDS']),6,x_limits,y_limits,resolution,
#                       'data/fields/',goal=data['GOAL'])
# =============================================================================

# =============================================================================
#     fields = []
#     for a in angles : 
#         f = firing_fields_ovc_agent(scene, network, weights, a)    
#         fields.append(f)
# 
#     np.save(save_location, fields)
# =============================================================================
    
    data = np.load('fields_1.npy')
    draw_ovc_allocentric(data,8,x_limits,y_limits,resolution,'data/fields/',hd=60)
