...
def generate_from_radial_grid(world, radius, grid_size, angle, 
                              images_file) : 
    rad   = np.linspace(0, radius, num=grid_size[0])
    theta = np.linspace(0, 2*np.pi, num=grid_size[1])
    
    rr,tt = np.meshgrid(rad,theta)
    grid_points = np.vstack((rr.flatten(),tt.flatten())).T
...
def get_activity_map(images, model, dense_layer_idx=8, network_from_file=True,
...
def get_activity_map_lesion(images, model, dense_layer_idx=10, network_from_file=True,
    ...
    if network_from_file : 
        dense_layer_activations = K.function([model.layers[1].input, model.layers[5].input],
                                             [model.layers[dense_layer_idx].output])
...
def mean_std_reward(training_curves) : 
    ...
    runs """
    all_rewards = np.stack(([training_curves[i]['episode_reward'][:1999] for i in range(len(training_curves))]))
