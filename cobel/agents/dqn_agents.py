...
class DQNAgentBaseline(AbstractRLAgent):
    ...
    class callbacksDQN(callbacks, callback_keras.Callback):
        ...
        def on_step_begin(self, step, logs):
            ...
            | logs:                         The trial log.
            '''
            self.rl_parent.current_step += 1
            logs['current_step'] = self.rl_parent.current_step
    ...
    def __init__(self, interface_OAI, memory_capacity=1000000, epsilon=0.3, model=None, 
                 gamma=0.8, weights=None, custom_callbacks={}): 
    ...
    def compile_agent(self, optimizer=Adam(lr=.001,),metrics=['mse']) : 
        ...
        '''
        #construct the agent
        self.agent = DQNAgent(model=self.model, nb_actions=self.number_of_actions, memory=self.memory, gamma=self.gamma, nb_steps_warmup=100, enable_dueling_network=False,
        ...
        if self.weights is not None :
            self.agent.load_weights(self.weights)
    ...
    def train(self, number_of_trials=100, max_number_of_steps=100, replay_batch_size=32):
        ...
        self.agent.fit(self.interface_OAI, nb_steps=self.max_steps, verbose=0, 
                       callbacks=[self.engaged_callbacks], nb_max_episode_steps=max_number_of_steps, 
                       visualize=False)
    
    def test(self, number_of_trials=100, max_number_of_steps=100, weights=None):
        '''
        ...
        # call the fit method to start the RL learning process
        history = self.agent.test(self.interface_OAI, nb_episodes=number_of_trials, verbose=0, 
                        callbacks=[self.engaged_callbacks], nb_max_episode_steps=max_number_of_steps, 
                        visualize=False)
