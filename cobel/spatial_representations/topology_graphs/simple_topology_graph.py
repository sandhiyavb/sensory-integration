...
class GridGraph(AbstractTopologyGraph) : 
    ...
    def __init__(self, n_nodes_x=5, n_nodes_y=5, n_neighbors=4,
                 ...
                 observation_module=None, noise=0.0) : 
    ...
    def init_visual_elements(self) :
        ...
        if self.visual_output:
            ...
            self.symbolBrushes = [qg.mkBrush(color=(128, 128, 128))]*len(self.nodes)
            ...
            for node in self.nodes:
                ...
                if node.startNode:
                    self.symbolBrushes[node.index] = qg.mkBrush(color=(0, 255, 0))

                # goal node is red
                if node.goalNode:
                    self.symbolBrushes[node.index] = qg.mkBrush(color=(255, 0, 0))
            ...
            self.topologyGraph.setData(pos=np.array(tempNodes), adj=np.array(
                tempEdges), symbolBrush=self.symbolBrushes)
    ...
    def set_start_nodes(self, start_nodes) : 
        #first remove current start_nodes
        for node_idx in self.start_nodes :
            self.nodes[node_idx].startNode = False
        #set new start nodes
        self.start_nodes = start_nodes
        for node_idx in self.start_nodes :
            self.nodes[node_idx].startNode = True
        print("Start node(s) set to : ", self.start_nodes)
        if self.visual_output:
            self.update_visual_elements()
