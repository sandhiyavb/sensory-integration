#run simulation for image only network
# basic imports
import os
import numpy as np
...
visual_output = True
switch_criterion_at = 300
network = "../../networks/image_network.json"
...
pref = 'data/stackman/'
weights_pref = 'data/stackman/'

def switch_criterion(logs) :
    trial    = logs['trial']
    rl_agent = logs['rl_parent']
    topology = rl_agent.interface_OAI.modules['spatial_representation']
    if trial%switch_criterion_at == 0 : 
        print(trial)
        topology.set_start_nodes([2,9,13,20]) #centers
...
def single_run(run=1):
    ...
    modules['spatial_representation'] = HexagonalGraphAllocentric(n_nodes_x=5, n_nodes_y=5,
                                                       n_neighbors=6, start_nodes=[2], goal_nodes=[22],
                                                       visual_output=True, 
    ...
    rl_agent = DQNAgentBaseline(modules['rl_interface'], 3000, 0.3, model=model, 
                                weights=weights_pref+'run_'+run+'/weights/'+'weights_trial_300.h5',
