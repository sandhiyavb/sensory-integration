...
df = pd.DataFrame(columns=['run','ablated_units', 'trial','steps','reward']) 
...
def get_units_by_type(unit_csv_path, run, unit_type):
...
def ablate_units_in_model(model, layer_name, unit_indices):
...
def perturb_units_multiplicative(model, layer_name, unit_indices, std=0.1):
    layer = model.get_layer(layer_name)
    W, b = layer.get_weights()
    
    for idx in unit_indices:
        scale = np.random.normal(1.0, std, size=W[:, idx].shape)
        W[:, idx] *= scale
        b[idx] *= np.random.normal(1.0, std)
    
    layer.set_weights([W, b])
    
def perturb_units_in_model(model, layer_name, unit_indices, std=0.1):
    layer = model.get_layer(layer_name)
    W, b = layer.get_weights()
    
    for idx in unit_indices:
        W[:, idx] += np.random.normal(0, std, size=W[:, idx].shape)
        b[idx] += np.random.normal(0, std)
    
    layer.set_weights([W, b])
    
def single_run(run=1, noise=0.1, add_noise_to='vector', noise_type='distributed', 
               input_type='both', unit_type=None):
    '''
    ...
    data_folder = f"../data/{simtype}/noise_simulations/{add_noise_to}_noise_{noise_type}/noise_{noise}/" 
    ...
    classified_csv = data_folder + f'classified_fields_{input_type}.csv'
    ...
    units = get_units_by_type(classified_csv, run, unit_type)
    print(f"Ablating {unit_type} units: {units}")
    #ablate_units_in_model(model, "analysis", units)
    perturb_units_in_model(model, "analysis", units, 0.5)
    
    #test agent
    rl_agent.test(number_of_trials, max_steps, weights=None)
    
    #log results incluuding which unit was silenced
    x = np.array(data_logger.training_data)
    df_metrics = pd.DataFrame(x, columns=["trial", "steps", "reward"])
    df_meta = pd.DataFrame({
    "run": [int(run)] * number_of_trials,
    "ablation_unit": [unit_type] * number_of_trials  # string column
})
    logs = pd.concat([df_meta, df_metrics], axis=1)
       
    logs.to_csv(data_folder+'test_log_group_ablation_noise_{}.csv'.format(input_type), 
                sep='\t', encoding='utf-8',index=False,
              mode='a', 
              header=not os.path.exists(data_folder+'test_log_group_ablation_noise_{}.csv'.format(input_type)))
