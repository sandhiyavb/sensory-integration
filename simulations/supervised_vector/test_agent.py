...
from custom_modules.observations.image_observations import ImageObservationFOV, ImagetoVectorEstimate
...
data_folder = 'data/supervised_model/'
     
def single_run(run=1, input_type='both'):
    '''
    ...
    vector_observation = ImagetoVectorEstimate(modules['world'], main_window,
                                                 imageDims=(72, 12), view_angle=240.0,
                                               prediction_model='vector_model.h5')
    ...
    if input_type=="random" : 
        ...
        rl_agent.test(number_of_trials, max_steps, 
                      weights=data_folder+"run_{}".format(run)+"/weights/weights_trial_4000.h5")
    ...
    logs.to_csv(data_folder+'test_log_{}.csv'.format(input_type), sep='\t', encoding='utf-8',index=False,
              mode='a', header=not os.path.exists(data_folder+'test_log_{}.csv'.format(input_type)))
