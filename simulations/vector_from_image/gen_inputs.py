...
def gen_agent_centered_images(world, angles, grid_size, obj="Cylinder",radial=False,
                              fov=fov, input_size = input_size) :
    """
    Specify a grid size and environment to generate 
    input images at grid points with (x,y) position.
    ...
    for angle in angles :   
        angle_images = []  
        for point in grid_points :
            world.teleportXY(obj, point[0], point[1])
            observation = world.step_simulation_without_physics(0.0,0.0,angle)
            image = observation[3].astype('float32')/255.0  
            angle_images.append(image)    
        resized_images = resize_images(angle_images, input_size[0], input_size[1])
        cropped_images = limit_field_of_view(resized_images, input_size[0], fov)        
        images.append(cropped_images)
...
def gen_agent_centered_vectors(world, angles, grid_size, radial=False) : 
    world_limits = world.getLimits()
    x_limits = [world_limits[0,0], world_limits[0,1]]
    y_limits = [world_limits[1,0], world_limits[1,1]]
    X = np.linspace(x_limits[0], x_limits[1], grid_size[0])
    ...
    for angle in angles :   
        angle_vectors = []  
        hd_rad = np.deg2rad(angle)
        for point in grid_points :            
            egocentric_angle = calculate_angle_ref(0.0,0.0, point[0], point[1],
            ...
            angle_vectors.append([distance, egocentric_angle])       
        vectors.append(angle_vectors)
    return np.array(vectors)


def gen_spatial_vectors(world, angles, grid_size, radial=False) : 
    world_limits = world.getLimits()
    x_limits = [world_limits[0,0], world_limits[0,1]]
    y_limits = [world_limits[1,0], world_limits[1,1]]
    X = np.linspace(x_limits[0], x_limits[1], grid_size[0])
    ...
    for angle in angles :   
        ...
        for point in grid_points :   
            ...
            egocentric_angle = calculate_angle_ref(point[0], point[1],0.0,0.0,
                                               ref=(np.cos(hd_rad), np.sin(hd_rad)))
            egocentric_angle = egocentric_angle / 360.0
            distance   = np.sqrt((point[0])**2 + (point[1])**2)
            distance = distance / max_distance #normalize distance
...
#image_data = gen_spatial_images(world, angles, resolution, noise=(0.0,0.1))
vector_data = gen_spatial_allocentric_vectors(world, angles, resolution,
                                              noise=(0.0,0.5))
#np.save('inputs/input_images_guidance_0.1.npy', image_data)
np.save('inputs/input_allo_vectors_guidance_0.5.npy', vector_data)
